...
// Get delivery mode ID from URL
$id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

// Get delivery mode data
$deliveryMode = $deliveryModeController->getById($id);

if (!$deliveryMode) {
    $_SESSION['error'] = 'Delivery mode not found';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            $result = $deliveryModeController->update($id, $name);
            if ($result['success']) {
...
<head>
    ...
    <title>Edit Delivery Mode - Admin Panel</title>
            ...
            <form method="POST" class="edit-form">
                <div class="form-group">
                    ...
                    <input type="text" 
                           id="name" 
                           name="name" 
                           value="<?php echo htmlspecialchars($deliveryMode['name']); ?>" 
                           required>
                </div>
